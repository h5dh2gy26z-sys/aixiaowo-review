...
- Workspace root (this machine): `E:\aixiaowo\review`
- Git branch: `codex/thread-first-workflow-refresh`
- Remote tracking branch: `origin/codex/thread-first-workflow-refresh`
- Wenxian app (screening system):
  - Backend: `wenxian/literature_screening/`
  - Frontend: `wenxian/literature_screening_web/`
  - Report module: `wenxian/literature_screening/separated_modules/formal_report_module/`
  - Local runtime: `wenxian/docker-compose.local.yml` (stable) and `wenxian/docker-compose.dev.yml` (dev)
- Review agent (Survey/Tutorial baseline): `review_agent/review_writing_baseline/`
- Review web app (UI + API for review-agent artifacts):
  - Backend: `review_agent/review_web/backend/` (FastAPI)
  - Frontend: `review_agent/review_web/frontend/` (Vue; built files served by backend)
  - One-command start (Windows): `start-review-web.ps1`
  - UX: default “工作台”提供导入/筛选/Phase1-4 的一条龙，上手不需要先理解文件树；“文件”模式保留给高级用户
  - PDF convention (Phase 2): `review_agent/review_projects/<slug>/05_evidence/pdfs/<paper_id>.pdf`
...
This round (prep: Survey/Tutorial writing baseline):

- Added `review_agent/review_writing_baseline/` with:
  - authoritative sources manifest + distilled notes (`review_agent/review_writing_baseline/sources/`)
  - artifact contract + cross-platform conventions (`review_agent/review_writing_baseline/specs/`)
  - measurable rubric + templates + table schemas (`review_agent/review_writing_baseline/baseline/`)
  - prompt building blocks (`review_agent/review_writing_baseline/prompts/`)
  - small tools: `review_agent/review_writing_baseline/tools/scaffold_project.py`, `review_agent/review_writing_baseline/tools/rubric_check.py`
- Added an integration design note and a bridge importer:
  - `review_agent/review_writing_baseline/specs/system_design_survey.md`
  - `review_agent/review_writing_baseline/tools/import_from_screening_output.py` (converts `deduped_records.json` + `screening_decisions.json` into `papers_canonical.csv` + `abstract_decisions.csv`)
- Updated the project template so scaffolding passes rubric blockers without requiring immediate imports (`project_template/02_import/papers_canonical.csv` header-only)
- Added `.gitignore` patterns for heavy review artifacts under `review_agent/review_projects/**`
- Reorganized the workspace root into two top-level systems:
  - `wenxian/` (existing screening app)
  - `review_agent/` (survey/tutorial review-writing baseline)
- Expanded the Survey/Tutorial artifact contract and rubric to support higher-quality outcomes:
  - taxonomy mapping (`paper_classification.csv`), full-text prioritization (`fulltext_priority.csv`), and table/figure planning (`table_figure_plan.yaml`)
  - added a stage playbook and reason-tag spec for screening decisions
- Wrote a concrete project plan for starting a new development thread: `review_agent/PROJECT_PLAN.md`

This round (implementation: Review Web MVP):

- Added Phase 1 tool (abstracts -> taxonomy + planning artifacts):
  - `review_agent/review_writing_baseline/tools/phase1_abstract_to_taxonomy.py`
- Added runnable UI + API for the review-writing workflow:
  - `review_agent/review_web/backend/` (FastAPI API + static serving)
  - `review_agent/review_web/frontend/` (Vue + Vite; built to `dist/`)
- Added a demo project with generated artifacts so the UI is usable immediately:
  - `review_agent/review_projects/demo_survey_web/`

Historical notes (previous rounds):
...
### Review Agent (Survey/Tutorial writing)

- Use `review_agent/PROJECT_PLAN.md` as the roadmap.
- Phase 1: abstracts → taxonomy + mapping + full-text priority + table/figure plan.
- Phase 2: full-text PDFs → `05_evidence/paper_cards/*.json` (driven by prioritized list).
- Later: add RAG evidence pointers + traceability checks.
...
### Primary (this workspace)

Wenxian app:

- `wenxian/docker-compose.local.yml`
- `wenxian/docker-compose.dev.yml`
- `wenxian/literature_screening/src/literature_screening/pipeline/run_pipeline.py`
- `wenxian/literature_screening/src/literature_screening/strategy/prompt_builder.py`

Review agent:

- `review_agent/PROJECT_PLAN.md`
...
- `C:\Users\heiba\.codex\skills\review-project-state\SKILL.md`
...
Survey/Tutorial writing baseline (new):

- `review_agent/review_writing_baseline/README.md`
- `review_agent/review_writing_baseline/sources/manifest.yaml`
- `review_agent/review_writing_baseline/baseline/rubric_survey.yaml`
- `review_agent/review_writing_baseline/specs/artifact_contract_survey.md`
- `review_agent/review_writing_baseline/specs/repo_conventions.md`
- `review_agent/review_writing_baseline/prompts/system_base.md`
- `review_agent/review_writing_baseline/tools/scaffold_project.py`
- `review_agent/review_writing_baseline/tools/rubric_check.py`
- `review_agent/review_writing_baseline/tools/import_from_screening_output.py`
- `review_agent/review_writing_baseline/specs/system_design_survey.md`
...
- `python review_agent/review_writing_baseline/tools/scaffold_project.py demo_survey --dest-root tmp_review_projects` (post-template update: blockers now pass)

Legacy verification commands below were recorded in older environments and may not apply to this workspace layout:
...
Wenxian (Windows PowerShell):

```powershell
cd E:\aixiaowo\review\wenxian
docker compose -f docker-compose.local.yml up -d --build
docker compose -f docker-compose.local.yml down
```

Review Web (Windows PowerShell):

```powershell
cd E:\aixiaowo\review
.\start-review-web.ps1
# then open: http://127.0.0.1:8731
```

Legacy (macOS `.command` scripts):
...
Current workspace layout uses:

- `wenxian/` for the screening system
- `review_agent/` for Survey/Tutorial review writing
...
- Read `review_agent/PROJECT_PLAN.md` first for the review-writing roadmap.
- Wenxian runtime commands assume `cd E:\aixiaowo\review\wenxian` first.
